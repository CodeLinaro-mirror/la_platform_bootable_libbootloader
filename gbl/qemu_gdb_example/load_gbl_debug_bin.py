...
class LoadGblDebugBinCommand(gdb.Command):
  ...
  def scan_all_threads(self):
    ...
    for inferior in gdb.inferiors():
      for thread in inferior.threads():
        thread.switch()
        if int(gdb.parse_and_eval("$rax")) == MAGIC:
  ...
  def invoke(self, arg, from_tty):
    ...
    print(f"GBL connected. Take image base address from $rcx: {rcx:#x}")
    ...
    offset = (rcx - 0x0000000140000000) % (1 << 64)
    gdb.execute(f"add-symbol-file {arg} -o {offset:#x}")
    gdb.execute("set $rax = 0")  # Exits the wait loop in `wait_gdb()`
